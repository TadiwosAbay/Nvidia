...
int main() {
    Features features(true, true, true, true, true, RoundingMode::roundToNearestEven, 3, 4);
    ...
    detected_features.print_report();

    if (features == detected_features) {
        std::cout << "Features are the same." << std::endl;
    } else {
        std::cout << "Features are different." << std::endl;
    }
